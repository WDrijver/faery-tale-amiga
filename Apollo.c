...
void ApolloLoad(const char *filename, UBYTE **buffer, ULONG *lenght, UWORD offset)
{
	...
	static ULONG file_size = 0;    					// length of file
	static ULONG file_read = 0;   					// bytes read from file
	...
	file_buffer = (UBYTE*)AllocMem(file_size+15, MEMF_FAST);
...
void ApolloShowFile(const char *filename, UBYTE **buffer_draw, UBYTE **buffer_live, ULONG lenght, UWORD offset, UWORD gfx_mode, UWORD gfx_modulo)
{
	...
	*((volatile UWORD*)0xDFF1E6) = (UWORD)(gfx_modulo); 
	*((volatile UWORD*)0xDFF1F4) = (UWORD)(gfx_mode);
	*((volatile ULONG*)0xDFF1EC) = (ULONG)(*buffer_live);
}

void ApolloPlayFile(const char *filename, UBYTE *buffer, UWORD offset, int channel, int volume_left, int volume_right, bool loop)
{
	static ULONG file_size = 0;
	static ULONG file_read = 0;
	static FILE *file_handle = 0; 
	...
	*((volatile ULONG*)(0xDFF400 + (channel * 0x10))) = (ULONG)buffer+offset;        	   	// Set Channel Pointer
	*((volatile ULONG*)(0xDFF404 + (channel * 0x10))) = (ULONG)(((file_size-offset)/8)-32);	// Set Channel Music Lenght (in pairs of stereo sample = 2 * 2 * 16-bit = 64-bit chunksize = filesize in bytes / 8)
...
void ApolloCacheFile(const char *filename, UBYTE **cache, ULONG *lenght, UWORD file_offset)
{
	static ULONG file_size = 0;
	static ULONG file_read = 0;
	static FILE *file_handle = 0; 
...
bool ApolloPlay(int channel, int volume_left, int volume_right, bool loop, UBYTE *buffer, ULONG lenght, UWORD offset)
{
	...
	*((volatile ULONG*)(0xDFF400 + (channel * 0x10))) = (ULONG)buffer+offset;        	   	// Set Channel Pointer
	*((volatile ULONG*)(0xDFF404 + (channel * 0x10))) = (ULONG)(((lenght-offset)/8)-32);	// Set Channel Music Lenght (in pairs of stereo sample = 2 * 2 * 16-bit = 64-bit chunksize = filesize in bytes / 8)
...
{
	#ifdef APOLLODEBUG_APOLLO
	DebugPutDec("ApolloStop: Fading audio on channel", channel);
	#endif
...
void ApolloShow(UWORD gfx_mode, UWORD gfx_modulo, UBYTE *buffer)
{
	#ifdef APOLLODEBUG_APOLLO
		sprintf(ApolloDebugText, "ApolloShow: gfxmode = %x | gfx_modulo = % d | buffer = %d |\n", gfx_mode, gfx_modulo, (ULONG)(buffer));
		DebugPutStr(ApolloDebugText);
	#endif
	*((volatile UWORD*)0xDFF1E6) = (UWORD)(gfx_modulo); 
	*((volatile UWORD*)0xDFF1F4) = (UWORD)(gfx_mode);
	*((volatile ULONG*)0xDFF1EC) = (ULONG)(buffer);
}
...
void ApolloMouse(ApolloMouseState *MouseState)
{
	...
	MouseButtonLeft_Value	=  *((volatile UBYTE*)APOLLO_MOUSE_BUTTON1);
	if ((MouseButtonLeft_Value & 0x40) == 0) MouseState->Mouse_Button1 = true;

	MouseButtonRight_Value	= *((volatile UWORD*)APOLLO_MOUSE_BUTTON2);
	...
	if (MouseState->Mouse_X_Delta != 0 || MouseState->Mouse_Y_Delta != 0)
	{
		...
		*((volatile UWORD*)APOLLO_POINTER_SET_X) = (UWORD)(MouseState->Mouse_X_Position) + 16; 
		*((volatile UWORD*)APOLLO_POINTER_SET_Y) = (UWORD)(MouseState->Mouse_Y_Position) +  8;  
...
void ApolloKeyboard(unsigned char *Keyboard_Key)
{
	...
	Keyboard_Now = (UBYTE) ((Keyboard_Raw>>1) | (Keyboard_Raw<<7));		// ror.b #1
